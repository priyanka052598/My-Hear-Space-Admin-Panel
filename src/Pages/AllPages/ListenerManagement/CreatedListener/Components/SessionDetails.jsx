...
function SessionDetails({ listner }) {
  const [sessionHistory, setsessionHistory] = useState()


  const getListenerHistory = async () => {
    try {
      ...
      const response = await axios.get(
        `${API_ENDPOINTS.getListnerSessionHistory}${listner._id}`,
        {
          headers: {
    ...
    <div className="px-7 mt-8 flex gap-7">
      <div className=" w-[60%] rounded-[24px] mb-20 pb-8  bg-white  flex flex-col">
        ...
        <div className="  overflow-y-auto h-[500px] flex flex-col gap-5  px-[40px] pb-[30px]">
          {sessionHistory?.map((item) => {
            return (
              <div className="bg-[#F0F0F0] rounded-[12px] flex justify-between w-full px-[24px] py-[16px]">
                <div className="flex items-center gap-3">
                  <div className="bg-[#D9D9D9] p-2 w-fit rounded-full">
                    ...
                    {item.type === "Declined" && (
                      <IoBan className="text-[20px] text-[#FF5D5D]" />
                    )}
                  </div>{" "}
                  <div>
                    <h2
                      className={`${
                        item.type == "Missed" || item.type == "Declined"
                          ? "text-[#FF5D5D]"
                          : "text-black"
                      }`}
                    >
                      {item.conversationType == "phoneCall"
                        ? "Phone Call"
                        : "Video Call"}{" "}
                    </h2>
                    <div
                    ...
                    >
                      {" "}
                      {item.status.startsWith("2") && <GoClock />}
                      {item.status}
                    </div>
                  </div>
                </div>
                <div className="flex flex-col">
                  <span className="text-[20px] self-end font-500">
                    ₹{item.coinsDeducted}
                  </span>
                  <span className="text-[15px] font-400">{item.startTime.slice(0, 10)}</span>
                </div>
              </div>
            );
          })}
      ...
      <div className="w-[40%]">
        ...
        <div className=" flex flex-wrap w-full  gap-5">
          ...
          <div className="balance  w-[45%] bg-white rounded-[24px] flex flex-col justify-center items-center gap-[16px] px-[12px] py-[24px]">
            <div className="p-4 bg-[#F0F0F0]  rounded-[12px]">
              <FaPhoneSlash className="text-[40px]" />
            </div>
            <div className="flex flex-col gap-[4px]">
              <span className="text-[28px] text-black text-center">
                ₹250.45
