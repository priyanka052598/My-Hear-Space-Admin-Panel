...
const {item} = location.state
      ...
      const [uploadedImage, setUploadedImage] = useState(item.blogThumbnail);
         const [imageName, setImageName] = useState(""); // State to store the uploaded file name
         const [sections, setSections] = useState(item.additionalSections);
         const[showPopup,setShowPopup]=useState()
          const [blogTitle, setBlogTitle] = useState(item.title);
              const [blogTag, setBlogTag] = useState(item.tag);
    ...
    let response =    await axios.put(API_ENDPOINTS.updateBlog + item?._id, blogData, {
