    ...
    const UpdateRequest = async () => {
      try {
        ...
        const response = await axios.put(
          `${API_ENDPOINTS.UpdateWithdrawlRequestStatus}/${state?.listenerId}`,
          {
            status:"Successful",
                    ...
                    <div className="  flex flex-col rounded-[24px] bg-white px-[40px] py-[30px] ">
                      ...
                      <div className="grid grid-cols-[250px_1fr] gap-0 text-left border border-gray-300">
                        ...
                        <div className="text-gray-900">
                          <div className="px-4 py-3 border-b text-[#FF5D5D]">
                            {state?.status}
                          </div>
                          <div className="px-4 py-3 border-b">
                ...
                <button
                  onClick={() => {
                    if (uploadedImage) {
                        UpdateRequest()
                    //   setSubmitted(true);
                    } else toast.error("Please upload image first");
                  }}
                  className={`text-[18px] text-white mr-7 px-6 py-2 ${
                    uploadedImage ? "bg-[#3A3A3A]" : "bg-[#D9D9D9]"
                  } rounded-[8px]`}
                  type="button"
                >
                  {" "}
